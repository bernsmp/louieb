...
// Fallback list used when the newsletter_entries Supabase table is empty or
// missing (i.e., before the migration + seed has been run). Once the DB has
// any rows, those take over and this list is ignored.
const SUNDAY_STARTERS_FALLBACK = [
...
async function getNewsletterEntries(): Promise<{ title: string; url: string }[]> {
  if (!supabaseAdmin) return SUNDAY_STARTERS_FALLBACK;
  const { data, error } = await supabaseAdmin
    .from("newsletter_entries")
    .select("title, url")
    .order("display_order", { ascending: true });
  if (error || !data || data.length === 0) {
    // Table doesn't exist yet, or is empty — fall back to the hardcoded list
    // so /site-map keeps showing the full newsletter archive. Once the
    // newsletter_entries table is populated via /cms/newsletter-entries, the
    // DB list takes over.
    return SUNDAY_STARTERS_FALLBACK;
  }
  return data;
}
...
const SALES_GUIDES = [
  ...
  { title: "How Should a Founder Treat Their Employees?", url: "/how-should-a-founder-treat-their-employees" },
...
export default async function SiteMapPage() {
  const [pageData, articles, blogPosts, videos, newsletterEntries] = await Promise.all([
    getSiteMapPageData(),
