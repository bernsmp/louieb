...
export default function BuildSalesPipelinePage() {
  ...
  const pipelineLayers = [
    {
      number: '01',
      title: v('layer1Title', 'Define What a Qualified Opportunity Actually Looks Like'),
      detail: v('layer1Detail', "Most founders at $1M–$5M ARR have never written down their ICP with hard criteria. \"Anyone who could use our product\" is not a definition — it's an excuse to avoid the harder work of filtering. A qualified opportunity has four attributes: the right company size, the right trigger event, the right job title in the room, and a budget window that matches your sales cycle. Without these, your pipeline fills with noise that kills forecasting accuracy."),
      fix: v('layer1Fix', "Pull your last 20 closed-won deals. Identify the three attributes they share that your losses don't. Those become your qualification criteria. Add them as required CRM fields before a deal can move past Stage 1."),
    },
    {
      number: '02',
      title: v('layer2Title', 'Build Three Separate Pipeline Inputs — Not Just One'),
      detail: v('layer2Detail', "Referral networks feel like pipeline, but they're disguised dependency. When referrals dry up — and they always do at some point — there's nothing underneath. A self-filling pipeline has three independent inputs running in parallel: outbound (you go find them), inbound (they find you), and partner/referral (warm introductions from existing relationships). Each requires different infrastructure and different ownership."),
      fix: v('layer2Fix', "Audit how your last 30 closed-won deals entered your pipeline. If more than 60% came from one source, that's a single point of failure. Build the other two inputs as parallel programs, even at small scale."),
    },
    {
      number: '03',
      title: v('layer3Title', 'Set Stage Exit Criteria Based on Buyer Milestones, Not Seller Actions'),
      detail: v('layer3Detail', "The most common pipeline problem isn't deal quality — it's that pipeline stages are defined by what the seller does (\"sent proposal,\" \"had call\") instead of what the buyer commits to. Deals stuck in \"proposal sent\" for 45 days aren't in your pipeline — they're in your wishlist. Buyer milestone stages look different: \"confirmed budget and timeline,\" \"economic buyer identified and engaged,\" \"legal review initiated.\" These stages are honest about where deals actually are."),
      fix: v('layer3Fix', "Rewrite your pipeline stages so that each stage advance requires a documented buyer action or commitment — not just a seller activity. Purge any deal that can't pass its current stage's exit criteria."),
    },
    {
      number: '04',
      title: v('layer4Title', 'Calculate Minimum Pipeline Coverage for Your Number'),
      detail: v('layer4Detail', "Most founders don't know how much pipeline they actually need to hit quota. The formula is simple: take your revenue target, divide by your close rate, and that's your minimum pipeline value at Stage 1. If you close 25% of qualified opportunities and need $2M in new revenue, you need $8M in qualified pipeline. If you don't have that number and no plan to build it, your revenue target is a hope, not a plan."),
      fix: v('layer4Fix', "Run the pipeline coverage calculation for your current quarter. If you're below 3x coverage on committed revenue, you have a pipeline generation problem — not a closing problem. That changes where you focus your effort."),
    },
    {
      number: '05',
      title: v('layer5Title', 'Establish a Weekly Pipeline Review That Creates Accountability'),
      detail: v('layer5Detail', "Pipeline doesn't fill itself — it fills because someone is accountable to the number every week. The weekly pipeline review is the mechanism that creates that accountability. Done correctly, it takes 45 minutes and covers three things: what moved forward this week and why, what is stuck and what the specific next action is, and what new qualified opportunities entered the pipe. Done incorrectly, it's a status update that everyone tolerates and no one learns from."),
      fix: v('layer5Fix', "Separate deal inspection from status reporting. Ask five questions for every deal over $10K: What is the specific next step? When does it happen? Who owns it? What could kill this deal? Is the timeline still accurate? If you can't answer all five, the deal doesn't belong in the pipeline."),
    },
  ]

  const pipelineKillers = [
    {
      killer: v('killer1Killer', 'Letting unqualified deals stay in the pipeline "just in case"'),
      result: v('killer1Result', 'Inflated forecasts that destroy trust in your numbers'),
    },
    {
      killer: v('killer2Killer', 'Using activity-based stage definitions instead of buyer milestones'),
      result: v('killer2Result', 'Pipeline that shows progress when deals are actually stalling'),
    },
    {
      killer: v('killer3Killer', 'No written ICP criteria applied at point of entry'),
      result: v('killer3Result', 'Reps spending time on deals that were never real'),
    },
    {
      killer: v('killer4Killer', 'Relying on a single pipeline source (usually referrals)'),
      result: v('killer4Result', 'Volatile pipeline that disappears when referrals slow down'),
    },
    {
      killer: v('killer5Killer', 'Pipeline reviews that cover status but not next actions'),
      result: v('killer5Result', 'Meetings that feel productive but move nothing forward'),
    },
  ]
      ...
      <section className="bg-white py-16 md:py-20">
        <div className="mx-auto max-w-4xl px-6">
          ...
          >
            ...
            >
              {v('sectionLayersHeading', 'The 5-Layer Pipeline Framework')}
            </motion.h2>
            <motion.p variants={itemVariants} className="mb-10 text-center text-neutral-600">
              {v('sectionLayersSubheading', 'Each layer builds on the last. Skip one and the pipeline looks full but performs empty.')}
            </motion.p>
        ...
        <div className="mx-auto max-w-5xl px-6">
          ...
          >
            <motion.h2
              ...
              className="mb-4 text-center font-serif text-3xl font-bold text-neutral-900 md:text-4xl"
            >
              {v('sectionKillersHeading', '5 Pipeline Habits That Kill Growth')}
            </motion.h2>
            <motion.p variants={itemVariants} className="mb-10 text-center text-neutral-600">
              {v('sectionKillersSubheading', 'These are the patterns I find in the first pipeline audit on almost every engagement.')}
            </motion.p>
